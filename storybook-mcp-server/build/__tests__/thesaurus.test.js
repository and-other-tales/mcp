...
describe('ContextualThesaurus', () => {
    ...
    test('finds synonyms with context', () => {
        const result = thesaurus.findSynonyms('walked', 'John walked slowly down the street', 'It was a peaceful afternoon as John made his way through town.');
    ...
    test('ranks synonyms by context appropriateness', () => {
        const result = thesaurus.findSynonyms('said', '"I don\'t know," he said angrily.', 'The argument was getting heated.');
    ...
    test('handles dialogue context', () => {
        const result = thesaurus.findSynonyms('said', '"Hello," she said with a smile.', 'A friendly conversation was taking place.');
        expect(result.some(r => r.usageNotes.includes('formal'))).toBeTruthy();
    });
    test('considers narrative tone', () => {
        const result = thesaurus.findSynonyms('walked', 'The diplomat walked through the grand hall.', 'The formal reception was attended by various dignitaries.');
        expect(result.some(r => r.usageNotes.includes('formal'))).toBeTruthy();
    });
    ...
    test('provides meaningful usage notes', () => {
        const result = thesaurus.findSynonyms('angry', 'He was angry about the situation.', 'The tension in the room was palpable.');
        result.forEach(suggestion => {
