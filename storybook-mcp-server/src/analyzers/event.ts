...
import type { Event, EventAnalysisResult, EventChain, EventSequence, TimelineEvent, ContinuityError } from '../types';
...
export class EventContinuityAnalyzer {
  ...
  private timeKeywords = ['before', 'after', 'during', 'when', 'while', 'then', 'next', 'finally'];
  private eventKeywords = ['happened', 'occurred', 'took place', 'began', 'started', 'ended', 'finished'];

  /**
   * Analyzes event continuity throughout the manuscript
   */
  public analyzeEvents(text: string): EventAnalysisResult {
    try {
      this.events = [];
      ...
      const suggestions = this.generateEventSuggestions();      return {
        events: this.events,
   ...
   */
  private processEventParagraph(text: string, paragraphNumber: number): void {
    const doc = nlp(text);
   ...
   */
  private createEvent(text: string, paragraph: number): Event | null {
    const doc = nlp(text);
   ...
   */
  private extractEventLocation(doc: any): string | undefined {
   ...
   */
  private updateEventCharacters(doc: any, paragraphNumber: number): void {
   ...
   */
  private extractTimestamp(doc: any): string | undefined {
   ...
   */
  private checkEventContinuity(paragraph: number, text: string): void {
   ...
   */
  private checkLocationConsistency(events: Event[], paragraph: number): void {
   ...
   */
  private buildEventChain(): EventChain {
    const sequences: EventSequence[] = [];
    const timeline: TimelineEvent[] = [];
    const possiblePlotHoles: string[] = [];
   ...
   */
  private identifyPlotHoles(sequences: EventSequence[], plotHoles: string[]): void {
    // Check for unresolved events
    const eventConnections = new Map<string, Set<string>>();
   ...
   */
  private checkCharacterConsistencyAcrossSequences(sequences: EventSequence[], plotHoles: string[]): void {
      ...
      sequences.slice(i + 1).forEach(seq2 => {
        ...
        if (commonCharacters.length > 0) {
          ...
          if (!this.events.some(event => 
            event.characters.some(char => commonCharacters.includes(char)) &&
            ...
            plotHoles.push(
              `Characters ${commonCharacters.join(', ')} appear in ${locationChange} without clear travel or transition`
            );
          }
        }
      });
   ...
   */
  private generateEventSuggestions(): string[] {
   ...
   */
  private calculateEventDensity(): number {
   ...
   */
  private checkEventCharacterDistribution(suggestions: string[]): void {
    ...
    const avgEventsPerCharacter = 
      Array.from(characterEventCounts.values()).reduce((a, b) => a + b, 0) / 
