...
import type { ChunkingOptions } from '../types/chunking';
...
describe('Manuscript Management System Integration', () => {
  ...
  let chunkManager: ContextChunkManager;
  ...
  describe('Dynamic Prompting Integration', () => {
    it('should generate context-aware prompts based on manuscript chunks', () => {
      ...
      const contextWindow = promptManager.integrateChunkAnalysis(
        chunkManager.getContextForPosition(0)[0]
      );

      // Generate a prompt
      const prompt = promptManager.createPrompt(
        'edit',
        contextWindow.currentFocus,
        contextWindow,
        [{
          type: 'continuity',
